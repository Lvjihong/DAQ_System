...
public:
    ...
    void start_devices(const k4a_device_configuration_t &master_config,  k4a_device_configuration_t &sub_config)
    {
        ...
        for (k4a::device &d : subordinate_devices)
        {
            if (i == 0)
            {
                sub_config.depth_delay_off_color_usec = 80;
            }//第三台从机的延迟设置为240
            else
            {
                sub_config.depth_delay_off_color_usec = 240;
            }
    ...
    void stop_devices()//相机停止拍摄进入待机状态
    {
        for (k4a::device& d : subordinate_devices)
        {
            d.stop_cameras();
    ...
    std::vector<k4a::capture> get_synchronized_captures(k4a_device_configuration_t &sub_config,
                                                        bool compare_sub_depth_instead_of_color = false)
    {
        ...
        while (!have_synced_images)
        {
            ...
            std::chrono::microseconds master_color_image_time = master_color_image.get_device_timestamp();//主机时间戳
            ...
            for (size_t i = 0; i < subordinate_devices.size(); ++i)
            {
                ...
                {
                    std::chrono::microseconds sub_image_time = sub_image.get_device_timestamp();//从机时间戳
